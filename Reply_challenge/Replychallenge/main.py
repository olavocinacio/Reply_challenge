...
def main_iterator(width, height, building, antennas, reward):
    ...
    parse_output(int(len(antennas)), antennas)

    return results

def parse_output(total_antennas, antennas):
  textout = str(total_antennas)+'\n'

  used_antennas = []

  for i in range(0,total_antennas):

    #used_antennas[i][0] = i
    #used_antennas[i][1] = antennas[i][0]
    #used_antennas[i][2] = antennas[i][1]

  for line in used_antennas: # antenna aqui são as alocadas
      for element in line:
          textout += str(element)
          textout += ' '
      textout += '\n'
